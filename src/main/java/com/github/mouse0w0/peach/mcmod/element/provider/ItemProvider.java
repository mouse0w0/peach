...
public class ItemProvider extends ElementProvider<ItemElement> {
    ...
    public void index(Project project, ItemElement element, Indexer indexer) {
        ...
        List<ItemData> itemDataList = ImmutableList.of(new ItemData(itemId, 0, element.getMaxStackSize(), element.getDurability(), false, element.getDisplayName(), PreviewManager.getInstance(project).getItemImage(element)));
