...
public class LanguageTask implements CompileTask {
    ...
    public void run(Environment environment) throws Exception {
        Locale language = environment.getMetadata().getLanguage();
        String languageTag = language.getLanguage().toLowerCase() + "_" + language.getCountry().toLowerCase();
        try (BufferedWriter writer = environment.getAssetsFiler().newWriter("lang/" + languageTag + ".lang")) {
            LanguageGenerator.generate(environment.getMetadata().getId(), environment.getElements().values())
